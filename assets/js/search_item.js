...
function loadTabAjax(itemName) {
    ...
    $.ajax({
        type: "POST",
        url: $('input#btn-filter').val(),
        //async: false,
        data: {
            filterId: data_filter.filterId,
            limit: $("input#changeLimit").val()
        },
...
$('#btn_add_filter').click(function (event) {
    event.stopPropagation();
    var search = $('.input-group.margin .input-group-btn .dropdown-menu');
...
// $('body').on('click', '#btn_add_filter .btn_add_filter_click ul .btn_add_filter_click', function (event) {
//     event.stopPropagation();
//     $(this).closest('ul').slideUp(200);
// });

$('.btn_add_filter_click').click(function(event){
    ...
    if (id > 0) {
        ...
        bindFilter(id, 'records');
        $("#ulFilter li").removeClass('active');
        $("#ulFilter #liFilter_" + id).addClass('active');
    }else {
        $(this).parent().parent().find('ul').slideDown(200)
    }
    event.stopPropagation();
...
$(document).on('click','#btnShowModalAddFilter',function (event) {
    event.stopPropagation();
    $('#modalAddFilter .modal-content input').val('');
    $('#modalAddFilter .modal-content .table-bordered #container-filters-add tr').remove();
...
var elementClicked = false;
    ...
    $('#table-data').on('keydown', 'input.input-number', function (e) {
        ...
        if(elementClicked == false){
            $("select#selectData").html('<option value="one">' + itemIds.length + ' đã chọn</option><option value="all">Chọn tất cả (' + $("div.total-row").attr('data-count') + ')</option>');
        }
    ...
    $("body").on('click', 'select#selectData', function(){
        elementClicked = true;
        if($(this).val() === 'all'){
    ...
    function searchAjax() {
        $.ajax({
            ...
            success: function (response) {
                ...
                //load lại số dòng muốn thêm là bao nhiêu mac định là 20 
                $("input#changeLimit").val(response.limit);
    ...
    $('.dropdown-submenu a.btn-filter-data').on("click", function (e) {
        e.stopPropagation();
        var $this = $(this);
    ...
    $(window).click(function (e) {
        var search = $('.input-group-btn ul.dropdown-menu');
        if (search.has(e.target).length == 0 && !search.is(e.target)) {
            search.slideUp(200);
        }

    });
    $(window).click(function (e) {
        var listFilter = $('.input-group.margin #btn_add_filter>li>ul');
        if (listFilter.has(e.target).length == 0 && !listFilter.is(e.target)) {
            listFilter.slideUp(200);
        }
    });
    $('.input-group-btn ul li.dropdown-submenu>.btn-filter-data,.input-group-btn > ul > li > a.btn-filter-data-add').click(function (e) {
        e.stopPropagation();
    ...
    $('.input-group-btn ul li ul li .btn-filter-data,.input-group-btn  ul  li ul li .btn-filter-data-add').click(function (e) {
        e.stopPropagation();
        $(this).closest('.input-group-btn').find('ul.dropdown-menu').slideUp(200);
    });
    ...
    $('#remove-filter').click(function () {
        if (confirm('Bạn có chắc muốn xóa báo cáo này !')) deleteFilter();
    });
    // --------------------------- Thiết kế search ------------------------------- //
    ...
    $("body").on('keydown', '#changeLimit', function () {
        if (statusSearch != null) {
            clearTimeout(statusSearch);
            statusSearch = null;
        }
    })
    $("body").on('keyup', '#changeLimit', function () {
        if (statusSearch == null) {
            statusSearch = setTimeout(function () {
                searchAjax();
            }, 500);
...
//call_back được gửi xuống từ server khi thực hiện ajax nên call_back phải được định nghĩa global bên js trước khi server trả xuống
    ...
    // tính khoản cách tự động giữa các th trong table
    // với th đầu tiền dược tính = 5 %
    // var rowCount = $('#table-data tr th').length;
    // var avg =   95/ (rowCount -1 ); 
    // for(var i = 1; i< rowCount; i++){
    //      // $('#table-data tr th').eq(i).css('width', avg + "%");    
    //      $('#table-data tr th').eq(i).css('width',"500px");    
    // }
    // $(this).eq(0).css('width', "5%");
}
...
$.fn.Paginate = function (opt) {
    ...
    actions.render = function () {
        if (conf.pageSize == 1) {
            ...
            var totalData = '<div class="total-row" data-count="' + conf.totalRow + '">' + conf.totalDataShow + '/' + conf.totalRow + ' ' + conf.itemName + '</div>';
            $(".total-data").html(totalData);
            // $('.paginate_table').html(html);
            return false;
        }
        ...
        if (parseInt(conf.totalDataShow) < parseInt(conf.limit)) {
            totalDataShow = ((parseInt(conf.page) - 1) * parseInt(conf.limit)) + parseInt(conf.totalDataShow);
        } else {
            totalDataShow = parseInt(conf.limit) * parseInt(conf.page);
        }
        console.log("=====conf.page========", conf.page)
        console.log("=====conf.pageSize========", conf.pageSize)
        // html = '<div class="total-row">' + conf.totalRow + ' ' + conf.itemName + '</div>' + html;
        var totalData = '<div class="total-row" data-count="' + conf.totalRow + '">' + conf.totalDataShow + '/' + conf.totalRow + ' ' + conf.itemName + '</div>';
        html = '<div class="up-n-pager"><span>Trang '+conf.page+'/'+conf.pageSize+'</span>' + html;
        ...
        $('select#selectData option[value="all"]').text("Chọn tất cả ("+conf.totalRow+")");
    };
    actions.event = function () {
        // bắt sự kiện chuyển trang của table khi phân trang
        $("body").on('click', '.paginate_table a', function (e) {
            ...
            if (!$(this).hasClass('disable')) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        conf.limit = parseInt(response.limit);
                        $("input#changeLimit").val(response.limit);
                        actions.render();
                    },
                    error: function (response) {
