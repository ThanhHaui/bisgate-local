...
(function ($) {
    $.fn.CeoSlider = function (options) {
        var defaults = {
            lable_right : 'Đồng ý',
            lable_left : 'Không hủy nữa',
            lable_yes : 'Buông chuột để KHÔNG HUỶ NỮA',
            lable_no : 'Buông chuột để HUỶ',
            success: function() {},
            error : function () {}
        };
        ...
        this.append('<input type="range" min="0" max="1000" value="500" role="input-range"><p class="huy" id="tex"> '+settings.lable_left+'</p><p class="dong_y" id="tex"> '+settings.lable_right+'</p>');
        this.css('position', 'relative');
        rangeslider(this, settings);
        ...
        this.change(function () {
            ...
            if (value == 1000) {
                ...
                $(this).find('input[type="range"]').val(500).change();
            }

            if (value == 0) {
                settings.error.call(this, true);
                $(this).find('input[type="range"]').val(500).change();
            }
    ...
    function rangeslider(obj, settings) {
        $('input[type="range"]').rangeslider({
            polyfill: false,
            // Callback function
            onInit: function () {
                obj.find('.rangeslider--horizontal').css('background-color', 'rgba(241, 241, 241, 1)');
            },
            ...
            onSlide: function (position, value) {
                ...
                if (value > 500) {
                    obj.find('.rangeslider--horizontal').css('background-color', 'rgba(41, 109, 12, ' + color + ')');
                    obj.find('.rangeslider__handle').removeClass('on_the_right');
                    obj.find('.rangeslider__handle').addClass('on_the_left');
                    obj.find('.huy').css('color', '#fff');
                    obj.find('.dong_y').css('color', '#fff');

                }
                if (value < 500) {
                    var color_left = 1 - color;
                    obj.find('.rangeslider--horizontal').css('background-color', 'rgba(157, 17, 17, ' + color_left + ')');
                    obj.find('.rangeslider__handle').removeClass('on_the_left');
                    obj.find('.rangeslider__handle').addClass('on_the_right');
                    obj.find('.huy').css('color', '#fff');
                    obj.find('.dong_y').css('color', '#fff');
                ...
                if (value == 500) {
                    //rgba(125, 204, 196, 0.7)
                    obj.find('.rangeslider--horizontal').css('background-color', 'rgba(241, 241, 241, 1)');
                    obj.find('.huy').css('color', '#9d1111');
                    obj.find('.dong_y').css('color', '#377d19');
                    obj.find('.rangeslider__handle').removeClass('on_the_right');
                    obj.find('.rangeslider__handle').removeClass('on_the_left');
                }
                if (value < 1000) {
                    obj.find('.huy').text(settings.lable_left);
                }
                if (value > 0) {
                    obj.find('.dong_y').text(settings.lable_right);
                }
                if (value == 1000) {
                    obj.find('.rangeslider--horizontal').css('background-color', 'rgb(0, 118, 163)');
                    obj.find('.huy').text(settings.lable_yes);
                    obj.find('.huy').css('color', '#fff');
                    obj.find('.dong_y').fadeOut();
                }
                else{
                    obj.find('.dong_y').fadeIn();
                }


                if (value == 0) {
                    obj.find('.rangeslider--horizontal').css('background-color', 'rgb(0, 118, 163)');
                    obj.find('.dong_y').text(settings.lable_no);
                    obj.find('.dong_y').css('color', '#fff');
                    obj.find('.huy').hide();
                }
                else{
            ...
            onSlideEnd: function (position, value) {
                if (value < 1000 && value > 0) {
                    obj.find('input[type="range"]').val(500).change();
                }
