...
Class Mongo_db{
    ...
    private function connect()
    {
        ...
        try
        {
            $dns = "mongodb://{$this->hostname}:{$this->port}/{$this->database}?authSource=admin";
     ...
     * @usage : $this->mongo_db->where_gt('foo', 20);
     */
    public function where_compare($type = 0, $field = "", $x)
    {
     ...
     */
    public function where_between($field = "", $x, $y, $check = '')
    {

        if($x == 0 && $check != 'date_monitor') {
            return $this;
        }
        if (!isset($field))
        {
        ...
        $this->_w($field);
        if($check == 'date_monitor') {
            if($x !== 0) {
                $this->wheres[$field]['$gte'] = $this->date(strtotime($x) * 1000);
            }
            if($y !== 0) {
                $this->wheres[$field]['$lte'] =  $this->date(strtotime($y) * 1000);
            }
        } else if($check =='date' || $check =='time_min') {
            if($x == 1) {
                $this->wheres[$field]['$gte'] = $this->date(strtotime($y) * 1000);
            } else if($x == 2) {
                $this->wheres[$field]['$lte'] =  $this->date(strtotime($y) * 1000);
            } else {
                $this->wheres[$field]['$gte'] = $this->date(strtotime($x) * 1000);
    ...
    public function getNewObject($mongoId) {
     ...
     */
    public function get($collection = "", $offset = 0, $limit = 0)
    {
        if (empty($collection))
        {
        ...
        try{
            ...
            $options['skip'] = !empty($offset) ? (int)$offset : (int) $this->offset;
            $options['limit'] = !empty($limit) ? (int)$limit : (int) $this->limit;
    ...
    public function aggregate($collection, $operation)
    {
        ...
        $cursor = $this->db->executeCommand($this->database, $aggregate);
